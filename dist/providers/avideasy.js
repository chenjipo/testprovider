...
function avideasyDeliverCached(items, callback, runKey, deliveryGen) {
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        var topQuality = item.directQuality && item.directQuality.length ? item.directQuality[0].quality : 1080;
        console.log('[RN-Fetch][AVIDEASY-DELIVER] gen=' + deliveryGen + ' rank=' + item.rank + ' host=' + item.host + ' label=Server A' + item.rank + ' type=' + (streamMeta.type || 'direct') + ' quality=' + topQuality);
        libs.embed_callback(item.file, AVIDEASY_PROVIDER, item.host, streamMeta.quality, callback, item.rank, item.tracks, [{ file: item.file, quality: topQuality }], item.headers, streamMeta.type ? { type: streamMeta.type } : {});
    }
}
