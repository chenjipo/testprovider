...
function buildStormQualitiesFromMasterUrl(masterUrl) {
    ...
    return buildStormVariantFiles(basePath, queryString);
}
function buildStormVariantFiles(basePath, queryString) {
...
function buildStormQualitiesFromProxyUrl(proxyUrl) {
    if (!proxyUrl || proxyUrl.indexOf('vodvidl.site') < 0) {
        return [];
    }
    var qIdx = proxyUrl.indexOf('?');
    var queryString = qIdx >= 0 ? proxyUrl.substring(qIdx) : '';
    var basePath = qIdx >= 0 ? proxyUrl.substring(0, qIdx) : proxyUrl;
    basePath = basePath.replace(/\/playlist\.m3u8$/, '');
    if (basePath.indexOf('/1080/') >= 0 || basePath.indexOf('/720/') >= 0 || basePath.indexOf('/360/') >= 0) {
        return [{ file: basePath + queryString, quality: 1080 }];
    }
    return buildStormVariantFiles(basePath, queryString);
}
...
function stripProxyMetaQuery(directUrl) {
    var parsed = parseProxyQueryParams(directUrl);
    var base = directUrl.indexOf('?') >= 0 ? directUrl.substring(0, directUrl.indexOf('?')) : directUrl;
    if (!parsed.tailQuery) {
        return base;
    }
    return base + '?' + parsed.tailQuery;
}
function sanitizePlayUrl(directUrl) {
    if (!directUrl) {
        return directUrl;
    }
    if (directUrl.indexOf('headers=') >= 0 || directUrl.indexOf('host=') >= 0) {
        return stripProxyMetaQuery(directUrl);
    }
    return directUrl;
}
function sanitizeQualities(qualities) {
    ...
    for (var i = 0; i < qualities.length; i++) {
        out.push({
            file: sanitizePlayUrl(qualities[i].file),
...
function mapQualitiesForTag(stormMasterUrl, tag) {
    var qualities = buildStormQualitiesFromMasterUrl(stormMasterUrl);
    for (var i = 0; i < qualities.length; i++) {
        var file = qualities[i].file;
        if (tag.indexOf('cdn') === 0) {
            var cdn = unwrapStormToCdn(file);
            if (cdn) {
                qualities[i].file = cdn.directUrl;
            }
        }
        else if (tag.indexOf('storm') === 0 && tag.indexOf('full') < 0) {
            qualities[i].file = stripProxyMetaQuery(file);
        }
    }
    return qualities;
}
function buildPlayCandidates(stormMasterUrl) {
    var qualities = buildStormQualitiesFromMasterUrl(stormMasterUrl);
    if (!qualities.length) {
        return { candidates: [], qualities: [] };
    }
    var storm1080 = _.orderBy(qualities, ['quality'], ['desc'])[0].file;
    var parsed = parseProxyQueryParams(stormMasterUrl);
    var playbackHeaders = buildPlaybackHeaders(parsed.headersObj);
...
function finishVidlinkEmbed(file, provider, callback, tracks, qualities, headerDirect, metadata) {
    ...
    if (file.indexOf('.m3u8') < 0 && file.indexOf('vodvidl.site') >= 0) {
        var derived = buildStormQualitiesFromProxyUrl(file);
        if (!derived.length && file.indexOf('/playlist.m3u8') >= 0) {
            derived = buildStormQualitiesFromMasterUrl(file);
        }
        if (derived.length) {
    ...
    console.log('[RN-Fetch][VIDLINK-EMBED-VERSION] v35');
    console.log('[RN-Fetch][VIDLINK-PLAY] url=' + String(file).substring(0, 200) + ' referer=' + (headerDirect['referer'] || headerDirect['Referer'] || ''));
...
function processVidlinkStream(parseSearch, provider, callback, metadata) { return __awaiter(_this, void 0, void 0, function () {
    var headerDirect, directUrl, tracks, directQuality, qualityIndex, q, qualityItem, parseDirecturl, transformed, stormBase, stormQualities, pack, probeIdx, probeCandidate, probeBody, mappedQualities, parseDirectSize, textDirect, m3u8Data, directQualitySorted, dedupeKey;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                ...
                if (directUrl && isStormProxyUrl(directUrl)) {
                    dedupeKey = stormPlaylistDedupeKey(directUrl);
                    if (dedupeKey && getVidlinkEmbedDoneMap()[dedupeKey]) {
                        return [2];
                    }
                }
                headerDirect = buildHeaderDirect();
                tracks = parseTracks(parseSearch.stream.captions);
                getVidlinkState().pending = {
                    playlist: directUrl,
                    provider: provider,
                    callback: callback,
                    metadata: metadata,
                ...
                if (directUrl.indexOf('/playlist.m3u8') >= 0 && directUrl.indexOf('vodvidl.site') >= 0) {
                    pack = buildPlayCandidates(directUrl);
                    probeIdx = 0;
                    getVidlinkState().probeDelayed = false;
                    return [3, 1];
                }
                return [3, 5];
            case 1:
                if (!getVidlinkState().probeDelayed) {
                    getVidlinkState().probeDelayed = true;
            ...
            case 2:
                return [3, 11];
            case 11:
                if (!(probeIdx < pack.candidates.length)) {
                    return [3, 13];
                }
                probeCandidate = pack.candidates[probeIdx];
                return [4, libs.request_get(probeCandidate.file, probeCandidate.headers)];
            case 12:
                probeBody = _a.sent();
                ...
                return [3, 11];
            case 13:
                if (getVidlinkState().vlHls && getVidlinkState().vlHls.ok && getVidlinkState().vlHls.url) {
                    probeCandidate = {
                        file: stripProxyMetaQuery(getVidlinkState().vlHls.url),
                        headers: buildHeaderDirect(),
                        tag: 'storm1080-auth',
                    };
                    mappedQualities = mapQualitiesForTag(directUrl, probeCandidate.tag);
                ...
                if (!pack.candidates.length) {
                    console.log('[RN-Fetch][VIDLINK-PROBE] all failed, skip embed');
                    return [2];
                }
                console.log('[RN-Fetch][VIDLINK-PROBE] all failed, skip embed (no valid m3u8)');
                return [2];
            case 5:
                ...
                if (isStormProxyUrl(directUrl)) {
                    stormQualities = buildStormQualitiesFromProxyUrl(directUrl);
                    if (!stormQualities.length) {
                        stormQualities = buildStormQualitiesFromMasterUrl(directUrl);
                    }
                    if (stormQualities.length) {
                        directQualitySorted = _.orderBy(stormQualities, ['quality'], ['desc']);
                        console.log('[RN-Fetch][VIDLINK-M3U8] storm-derived=' + stormQualities.length + ' best=' + directQualitySorted[0].quality);
                        finishVidlinkEmbed(directQualitySorted[0].file, provider, callback, tracks, directQualitySorted, headerDirect, metadata);
                        return [2];
                    }
...
callbacksEmbed['vidlink-embed'] = function (dataCallback, provider, host, callback, metadata) { return __awaiter(_this, void 0, void 0, function () {
    var data, parseSearch, dedupeKey, embedDone, pending, playUrl, mappedQualities, e_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                ...
                if (data.step === 'vl-hls') {
                    getVidlinkState().vlHls = { url: data.url, ok: data.ok, preview: data.preview };
                    console.log('[RN-Fetch][VIDLINK-HLS-PROBE] ok=' + (data.ok || 0) + ' url=' + String(data.url || '').substring(0, 120) + (data.preview ? ' prev=' + data.preview : ''));
                    if (data.ok && data.url) {
                        pending = getVidlinkState().pending;
