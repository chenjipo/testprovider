...
libs.__shouldSyncVodLinks = function () {
    if (libs.__vodSyncFlushed || libs.__vodSyncReleasing) {
        return false;
    }
    return !!libs.__vodSyncStartMs;
};
...
libs.__shouldDeferWebview = function (provider) {
    ...
    if (slot.pumping || (slot.queue && slot.queue.length)) {
        return true;
    }
    return false;
};
...
console.log('[RN-Fetch][EMBED-CFG] sync-v20');
