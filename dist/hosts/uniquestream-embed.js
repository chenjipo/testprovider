...
function buildUniqueStreamEmbedScript() {
    return "(function(){var done=0;function pm(m){try{window.ReactNativeWebView.postMessage(JSON.stringify(m));}catch(e){}}function isMediacache(u){u=String(u||'');return u.indexOf('mediacache.cc')>=0||u.indexOf('hls.uniquestream.net')>=0;}function toMaster(u){u=String(u||'');if(u.indexOf('/video.m3u8')>=0)return u.replace('/video.m3u8','/master.m3u8');return u;}function postUrl(u,src){if(done||!u||!isMediacache(u))return;u=toMaster(u);done=1;pm({step:'us-url',url:u,source:src||'scan'});}function scanLetUrl(){if(done)return;var html=document.documentElement?document.documentElement.innerHTML:'';var m=html.match(/let\\s+url\\s*=\\s*['\"]([^'\"]+)/i);if(m&&m[1])postUrl(m[1],'scan');var m2=html.match(/https?:[^'\"\\s<>]+\\.m3u8[^'\"\\s<>]*/i);if(m2)postUrl(m2[0],'scan-m3u8');}function loadIframeSrc(src,srcTag){if(done||!src)return;if(src.indexOf('//')===0)src='https:'+src;fetch(src,{credentials:'include'}).then(function(r){return r.text();}).then(function(html){var m=html.match(/let\\s+url\\s*=\\s*['\"]([^'\"]+)/i);if(m&&m[1])postUrl(m[1],srcTag||'iframe');scanLetUrl();}).catch(function(){});}function clickServerBtn(){var btn=document.querySelector('.server-btn[data-num=\"1\"]')||document.querySelector('.server-btn')||document.querySelector('[data-num=\"1\"].server-btn');if(btn){try{btn.click();pm({step:'us-click'});}catch(e){}}}function runPageAjax(){if(done||!window.uniquestreamPlayer)return false;var p=window.uniquestreamPlayer;if(!p.nonce||!p.postId)return false;var type=location.pathname.indexOf('/episodes/')>=0?'tv':'mv';var body='action=uniquestream_player_ajax&nonce='+encodeURIComponent(p.nonce)+'&post='+encodeURIComponent(String(p.postId))+'&type='+encodeURIComponent(type)+'&nume=1';fetch(p.ajaxUrl||'/wp-admin/admin-ajax.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8','X-Requested-With':'XMLHttpRequest'},body:body,credentials:'include'}).then(function(r){return r.text();}).then(function(t){pm({step:'us-ajax',preview:String(t||'').substring(0,80)});if(!t||String(t).trim()==='-1'||String(t).trim()==='0')return;try{var j=JSON.parse(t);var embed=j.embed_url||'';var m=embed.match(/src=[\"']([^\"']+)/i);if(m&&m[1])loadIframeSrc(m[1],'page-ajax');}catch(e){}}).catch(function(err){pm({step:'us-ajax-err',preview:String(err&&err.message?err.message:err)});});return true;}function whenPlayerReady(fn){var n=0;var iv=setInterval(function(){if(done){clearInterval(iv);return;}if(window.uniquestreamPlayer&&window.uniquestreamPlayer.nonce&&window.uniquestreamPlayer.postId){clearInterval(iv);fn();return;}n++;if(n>120)clearInterval(iv);},250);}function hookNet(){if(window.__usHooked)return;window.__usHooked=1;pm({step:'us-hook-ready'});var fo=fetch;fetch=function(a,b){return fo(a,b).then(function(r){var s=typeof a==='string'?a:(a&&a.url?a.url:'');if(!done&&s.indexOf('.m3u8')>=0&&isMediacache(s)){var pick=toMaster(s);r.clone().text().then(function(t){if(t&&t.indexOf('#EXTM3U')===0)postUrl(pick,'fetch-m3u8');}).catch(function(){});}return r;});};}hookNet();pm({step:'us-boot',href:location.href});if(location.hostname.indexOf('hls.uniquestream.net')>=0){scanLetUrl();}else if(location.hostname.indexOf('uniquestream.net')>=0){whenPlayerReady(function(){clickServerBtn();runPageAjax();var r=0;var ar=setInterval(function(){if(done||r>8){clearInterval(ar);return;}runPageAjax();r++;},1500);});}else{scanLetUrl();}var n=0;var iv=setInterval(function(){scanLetUrl();n++;if(done||n>80)clearInterval(iv);},500);})();";
}
