...
function buildPloyanInjectScript(urix, mid, eid, sv) {
    ...
    return "(function(){var u='" + sUrix + "';function pm(m){try{window.ReactNativeWebView.postMessage(JSON.stringify(m));}catch(e){}}function b64dec(s){s=String(s||'').replace(/-/g,'+').replace(/_/g,'/');while(s.length%4)s+='=';return atob(s);}function hookNet(){if(window.__plyHooked)return;window.__plyHooked=1;pm({step:'hook-ready'});var fo=fetch;fetch=function(a,b){b=b||{};b.credentials='include';return fo(a,b).then(function(r){var s=typeof a==='string'?a:(a&&a.url?a.url:'');if(s.indexOf('/get/')>=0||s.indexOf('.m3u8')>=0||s.indexOf('/hls/')>=0){r.clone().text().then(function(t){pm({status:r.status,responseURL:s,responseText:t,source:'hook'});}).catch(function(){});}return r;});};var xo=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(m,u){this._plyUrl=u;return xo.apply(this,arguments);};var xs=XMLHttpRequest.prototype.send;XMLHttpRequest.prototype.send=function(){var x=this;x.withCredentials=true;x.addEventListener('load',function(){var u=x._plyUrl||x.responseURL||'';if(u.indexOf('/get/')>=0||u.indexOf('.m3u8')>=0||u.indexOf('/hls/')>=0){pm({status:x.status,responseURL:u,responseText:x.responseText||'',source:'xhr'});}});return xs.apply(x,arguments);};}if(u){try{if(!location.hash||location.hash.length<3){history.replaceState(null,'',location.pathname+location.search+'#'+u);pm({step:'hash-set',len:u.length});}else{pm({step:'hash-ok',len:location.hash.length});}}catch(e){pm({step:'hash-err',error:String(e)});}}hookNet();})();";
}
